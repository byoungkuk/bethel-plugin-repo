...
- `sermon-critic`을 4단계 Gate 파이프라인으로 재설계했다. Gate 0은 자수·이중 공백·
  ...
  100점으로 평가한다. Gate 1 통과 후에만 Gate 2에서 독립 청중 3인을 병렬 호출해
  2/3 다수결로 결함을 확정하며, Gate 3에서는 구조 점수와 청중 결함을 섞지 않고 병기한다.
- Claude 모델명과 Agent 호출을 Codex의 `gpt-5.6-terra`·`gpt-5.6-luna`·
  `gpt-5.6-sol` 및 `spawn_agent` 흐름으로 변환했다.
- 반복 가능한 Gate 0 검사를 `scripts/gate0.py`로 분리하고 JSON 위치 정보를 반환하도록 했다.
